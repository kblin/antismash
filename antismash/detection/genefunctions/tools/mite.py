...
class Results(FunctionResults[MiteHit]):
    ...
    @property
    def get_best_hits(self) -> dict[str, MiteHit]:
        return self.best_hits
...
def classify(cds_features: Iterable[CDSFeature], options: ConfigType,
    ...
    all_hits = diamond.parse_tabular_output(raw)
    ...
    for hit in all_hits:
        blast_hits_by_cds_name[hit.query_id].append(MiteHit(
            ...
            description=", ".join(dataset.entries[hit.reference_id].subfunctions),
