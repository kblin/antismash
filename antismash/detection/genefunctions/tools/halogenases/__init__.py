...
def prepare_data(logging_only: bool = False) -> list[str]:
    ...
    """
    failures = []
    base_path = path.get_full_path(__file__, "data")
    for profile in fdh.GENERAL_FDH_PROFILES:
        failures.extend(hmmer.ensure_database_pressed(os.path.join(base_path, profile.filename),
                                                      return_not_raise=logging_only))
    failures.extend(hmmer.ensure_database_pressed(fdh.ALL_FDH_PROFILES, return_not_raise=logging_only))
    return failures
