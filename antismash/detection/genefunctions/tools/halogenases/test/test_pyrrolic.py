...
class TestPyrrolic(unittest.TestCase):
    ...
    def test_score_below_cutoff(self):
        ...
        with patch.object(substrate_analysis, "retrieve_fdh_signature_residues",
            ...
            result = categorise_on_substrate_level(cds, [self.bad_pyrrole_hit])
            patched.assert_called_once_with(cds.translation, self.bad_pyrrole_hit, self.profile.motifs)
        assert not result

    def test_no_motif(self):
        cds = DummyCDS()
        with patch.object(substrate_analysis, "retrieve_fdh_signature_residues", return_value={}) as patched:
            result = categorise_on_substrate_level(cds, [self.strong_pyrrole_hit])
            patched.assert_called_once_with(cds.translation, self.strong_pyrrole_hit, self.profile.motifs)
        assert not result
    ...
    def test_conventional_mono_di(self):
        ...
        with patch.object(substrate_analysis, "retrieve_fdh_signature_residues",
            ...
            matches = categorise_on_substrate_level(cds, [self.strong_pyrrole_hit])
            match = matches[0]
