...
class TestIndolic(IndolicBase):
    def test_strong_trp_5(self):
        ...
        with patch.object(substrate_analysis, "retrieve_fdh_signature_residues",
            ...
            matches = categorise_on_substrate_level(DummyCDS(), [self.trp_5_hmm_result])
            patched.assert_called_once_with(
                cds.translation, self.trp_5_hmm_result, TRP_5.motifs,
            )
    ...
    def test_weak_trp_5(self):
        with patch.object(substrate_analysis, "retrieve_fdh_signature_residues",
            ...
            low_quality_hit = HalogenaseHmmResult(reference_id=TRP_5.profile_name, bitscore=380, evalue=1e-2, query_id="test", profile=TRP_5.profile_name)
            matches = categorise_on_substrate_level(DummyCDS(), [low_quality_hit])
        match = matches[0]
        assert match.profile == TRP_5.profile_name
    ...
    def test_trp_6(self):
        ...
        with patch.object(substrate_analysis, "retrieve_fdh_signature_residues",
            ...
            matches = categorise_on_substrate_level(DummyCDS(), [self.trp_6_7_hmm_result])
        match = matches[0]
    ...
    def test_categorise_substrate_good_match(self, _patched_consensus_sig):
        cds = DummyCDS()
        matches = categorise_on_substrate_level(cds, [self.trp_5_hmm_result])
        assert matches == [
