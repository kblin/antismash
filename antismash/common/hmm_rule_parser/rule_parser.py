...
class TokenTypes(IntEnum):
    ...
    def is_a_rule_keyword(self) -> bool:
        ...
        """
        return self.RULE <= self.value and self.value != self.TEXT # pylint: disable=comparison-with-callable
...
class DetectionRule:
    """ Contains all information about a rule, i.e.
            ...
            example: a sequence ID of an example record containing a cluster hit by the rule
    ...
    def __init__(self, name: str, category: str, cutoff: int, neighbourhood: int, conditions: Conditions,
                 description: str = "", example: "ExampleRecord" = None, superiors: List[str] = None,
    ...
    def reconstruct_rule_text(self) -> str:
        ...
        if self.example:
            comments += f"EXAMPLE {self.example} "
...
class ExampleRecord:
    ...
    def __init__(self, database: str, accession: str, version: int, start: int, end: int) -> None:
        if database not in ("NCBI", ):
            raise AttributeError(f"Invialid reference database {database}")
        ...
        if not 0 <= start <= end:
            raise AttributeError(f"Invaldid start {start} or end {end} values")

        self.start = start
        self.end = end

    def __str__(self) -> str:
        return f"{self.database} {self.accession}.{self.version} {self.start}-{self.end}"
...
class Parser:  # pylint: disable=too-few-public-methods
    ...
    def _parse_rule(self) -> DetectionRule:
        ...
        return DetectionRule(rule_name, category, cutoff, neighbourhood, conditions,
                             description=description, example=example, superiors=superiors, related=related)
    ...
    def _parse_example(self) -> ExampleRecord:
        """ EXAMPLE = EXAMPLE_MARKER ID ID DOT INT INT-INT"""
        ...
        return ExampleRecord(database, accession, version, start, end)
